...
const Notes = () => {
  ...
  const { course, notes, setNotes, user } = useContext(UserContext);
  ...
  useEffect(() => {
    const getNotes = async () => {
      try {
        const response = await axios.get("notes/course/" + course._id);
  ...
  return (
    <main>
      ...
      <ul className="grid grid-cols-1 font-semibold sm:grid-cols-2 lg:flex lg:items-center lg:justify-start lg:gap-16">
        <li className="p-1">Batch : {course.year}</li>
        <li className="p-1">Semester : {course.semester}</li>
        {user.userType === "student" && (
          <li className="p-1">Professor : {course.professor.name}</li>
        )}
        <li>
          <Link
